...
SAVE_DIR = "/n/data1/hms/dbmi/zitnik/lab/users/jb611/surface/run_330_mesh"
...
df = pd.concat((df[df['binder']==1], df[df['binder']==0].sample(4000, random_state=42))).copy()
df.to_csv("data/preprocessed/run330_sampled.tsv", sep='\t')
...
np.save(os.path.join(stats_path, 'surface_times'), surface_times)
np.save(os.path.join(stats_path, 'label_times'), label_times)